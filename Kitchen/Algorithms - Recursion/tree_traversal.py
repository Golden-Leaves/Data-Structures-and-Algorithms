...
def recursive_traverse(node):
    tree = {"value": node.value}
    tree["left"] = None if node.left is None else recursive_traverse(node.left)
    tree["right"] = None if node.right is None else recursive_traverse(node.right)
    return tree
...
print(json.dumps(recursive_traverse(tree.lookup(75))))
