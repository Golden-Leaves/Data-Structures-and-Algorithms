...
def traverse_postorder(node: Node,result:list):
    if node is None:
        return result
    traverse_postorder(node.left,result) # type: ignore
    traverse_postorder(node.right,result) # type: ignore
...
print(traverse_postorder(tree.root,[])) #type: ignore
