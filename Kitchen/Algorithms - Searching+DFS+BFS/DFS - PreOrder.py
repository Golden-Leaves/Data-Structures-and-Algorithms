...
def traverse_preorder(node: Node,result:list):
    if node is None:
        return result
    result.append(node.value)
    traverse_preorder(node.left,result) # type: ignore
    traverse_preorder(node.right,result) # type: ignore
    return result
# Build tree
...
print(traverse_preorder(tree.root,[])) #type: ignore
